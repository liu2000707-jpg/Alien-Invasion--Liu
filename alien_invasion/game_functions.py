...
def create_kingalien(kingalien, screen, type):
    aliens.empty()
    x = screen.dimension.get_rect().centerx
    boss = KingAlien(screen, x, type)
    kingalien.add(boss)

def create_fleets(screen, alien_last_y, scores, kingalien, aliens, elitealiens):
...
def ship_hit(stats, screen, alien_last_y, scores, ship_group, aliens, bullets_player, bullets_aliens, kingalien, elitealiens, sb, settings):
    ...
    if settings.ships_left > 0:
        ...
        pygame.display.flip()
        if stats.game_active:
            create_fleets(screen, alien_last_y, scores, kingalien, aliens, elitealiens)
